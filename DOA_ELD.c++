...
double ELD_cost(const vector<double>& P, int iter = 0, int max_iter = 1) {
    ...
    double penalty = 10000 * fabs(totalPower - Pd);  // strong fixed penalty
    return cost + penalty;
}
...
pair<double, vector<double>> DOA(int pop, int T, const vector<double>& lb, const vector<double>& ub, vector<double>& convergence) {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        for (int j = 0; j < pop; ++j) {
            ...
            for (int h = 0; h < k; ++h) {
                int idx = indices[h];
                double step = w * (cos((i + 1) * M_PI / T) + 1) *
                              dist(gen) * (0.5 + 0.5 * dist(gen)) *
                              (ub[idx] - lb[idx]);
            ...
            // Balance power after each update
            double totalPower = accumulate(x[j].begin(), x[j].end(), 0.0);
            if (fabs(totalPower - Pd) > 1e-3) {
                double scale = Pd / totalPower;
                for (double &p : x[j]) p *= scale;
            }
        }
        ...
        // Local search every 50 iterations
        if (i % 50 == 0) {
            vector<double> candidate = sbest;
            uniform_real_distribution<> local_dist(-0.05, 0.05);
            for (int j = 0; j < D; ++j) {
